...
class B2BAddress(models.Model):
    ...
    purchase_id = models.CharField(max_length=100, blank=True, default='')
    allocation_id = models.CharField(
        max_length=100,
        unique=True,
        default='',
        error_messages={
            'unique': 'آدرس بازارگاه با این شماره تخصیص قبلاً ثبت شده است.'
        }
    )
    ...
    product = models.ForeignKey(
        'core.Product', on_delete=models.CASCADE, null=True)
    customer = models.ForeignKey(
        'core.Customer', on_delete=models.CASCADE, null=True)
    receiver = models.ForeignKey(
        'core.Receiver', on_delete=models.SET_NULL, null=True, blank=True)

    total_weight_purchased = models.DecimalField(
    ...
    purchase_weight = models.DecimalField(
        max_digits=20, decimal_places=0, default=0)
    waybilled_weight = models.DecimalField(
        max_digits=20, decimal_places=0, default=0)
    non_waybilled_weight = models.DecimalField(
        max_digits=20, decimal_places=0, default=0)
    agreement_period_1 = models.CharField(
        max_length=100, blank=True, default='')
    agreement_amount_1 = models.CharField(
        max_length=100, blank=True, default='')
    agreement_period_2 = models.CharField(
        max_length=100, blank=True, default='')
    agreement_amount_2 = models.CharField(
        max_length=100, blank=True, default='')
    agreement_period_3 = models.CharField(
        max_length=100, blank=True, default='')
    agreement_amount_3 = models.CharField(
        max_length=100, blank=True, default='')
    ...
    def __str__(self):
        customer_name = self.customer.company_name if self.customer and self.customer.customer_type == 'corporate' else (
            self.customer.full_name if self.customer else 'No Customer')
...
class B2BDistribution(models.Model):
    ...
    warehouse_receipt = models.ForeignKey(
        'warehouse.WarehouseReceipt', on_delete=models.PROTECT, limit_choices_to={'receipt_type': 'import_cottage'})
    sales_proforma = models.ForeignKey(
        'finance.SalesProforma', on_delete=models.PROTECT, null=True, blank=True)
...
class B2BSale(models.Model):
    ...
    cottage_code = models.CharField(max_length=100, blank=True, default='')
    ...
    offer = models.ForeignKey(B2BOffer, on_delete=models.CASCADE,
                              related_name='b2b_sales', null=True, blank=True)
    sales_proforma = models.ForeignKey(
        'finance.SalesProforma', on_delete=models.SET_NULL, related_name='b2b_sales', null=True, blank=True)
